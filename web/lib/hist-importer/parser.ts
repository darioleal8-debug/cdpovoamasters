...
import type { HistExtractedPlayer, HistGameMeta } from "./types";
...
function buildSystemPrompt(teamNameOverride?: string): string {
  const teamSection = teamNameOverride
    ? `TEAM TO EXTRACT: The user explicitly identified the team as "${teamNameOverride}". Extract ALL players from this team section. This is definitive.`
...
3. If neither found: extract ALL player rows from the first complete team section in the document.
Extract ONLY that team's players. Set team_name to the exact name as it appears in the document.`;

  return `You are a basketball box score parser specialized in Portuguese basketball (Liga INATEL / FPB / Federação Portuguesa de Basquetebol).

TASK: Extract the CD Póvoa / Póvoa Masters team player statistics AND game metadata from a historical box score document.

${teamSection}
Ignore the opposing team's player rows.

GAME METADATA:
• game_date: ISO "YYYY-MM-DD" if visible; null if absent
• opponent_name: opposing team's full name
• home_score: CD Póvoa's final score (integer or null)
• away_score: opponent's final score (integer or null)
• competition: league/cup name if visible (e.g. "Liga INATEL", "Distrital", "Taça"); null if absent
• team_name: CD Póvoa name as it appears in the document

── FIELD GOAL COLUMN RULES ─────────────────────────────────────────────────
RULE A — Explicit 2P columns ("2PM"/"2PA", "2PC"/"2PA", "2P C/A", "2P A/C"):
  → Use directly as fg2_made / fg2_att.
  → Use 3PM/3PA as fg3_made/fg3_att.
  → FGM/FGA (combined total) — IGNORE for fg2.

RULE B — Only FGM/FGA + 3PM/3PA (no explicit 2P column):
  → fg2_made = FGM − 3PM, fg2_att = FGA − 3PA.

RULE C — Only FGM/FGA, no 3P column:
  → Use as fg2_made/fg2_att, set fg3=0.

── COLUMN ALIASES ──────────────────────────────────────────────────────────
• "2P C/A", "2PC"/"2PA": fg2_made/fg2_att
• "3P C/A", "3PC"/"3PA": fg3_made/fg3_att
• "LL C/A", "LLC"/"LLA", "FTM"/"FTA": ft_made/ft_att
• "RO","R.Of.","OREB","ORB": reb_off  |  "RD","R.Def.","DREB","DRB": reb_def
• "ASS","As.","AST": ast  |  "INT","Ro.","STL": stl  |  "BLO","Bl.","BLK": blk
• "PER","To.","TOV","TO": tov  |  "FC","Fa.","PF": fouls_committed
• "FS","PFD","PFR": fouls_drawn (0 if absent)
• "MIN","Minutos": seconds_played → "MM:SS"→MM×60+SS, "MM"→MM×60, "DNP"/"NJ"/"-"→0
• "+/-": plus_minus (null if absent)

── IGNORE THESE COLUMNS ────────────────────────────────────────────────────
SR, PIR, EFF, CAS, TJS, PUJS, SBJS, FAJS, FJS, HS, L, DL, D, PB, AO, JS, TV, DD, BC, BP, BH — not stored.

── OCR NORMALIZATION ───────────────────────────────────────────────────────
Correct OCR errors in names (0→O, 1→I, rn→m). Restore accented chars (ã, ç, é, etc.).

VALIDATION: pts = fg2_made×2 + fg3_made×3 + ft_made. If mismatch, trust pts column.
...
function extractJSON(text: string): RawOutput {
  ...
  for (const fn of strategies) {
    try { return JSON.parse(fn()) as RawOutput; } catch { /* next */ }
  }
...
function normalizePlayer(p: Record<string, unknown>): HistExtractedPlayer {
  return {
    pdf_name:        String(p.pdf_name ?? ""),
    pdf_number:      p.pdf_number != null ? toInt(p.pdf_number) : null,
    seconds_played:  toInt(p.seconds_played),
    fg2_made:        toInt(p.fg2_made),   fg2_att: toInt(p.fg2_att),
    fg3_made:        toInt(p.fg3_made),   fg3_att: toInt(p.fg3_att),
    ft_made:         toInt(p.ft_made),    ft_att:  toInt(p.ft_att),
    reb_off:         toInt(p.reb_off),    reb_def: toInt(p.reb_def),
    ast:             toInt(p.ast),        stl:     toInt(p.stl),
    blk:             toInt(p.blk),        tov:     toInt(p.tov),
...
export interface HistParseResult {
  players:  HistExtractedPlayer[];
  gameMeta: HistGameMeta;
  aiLog:    string;
  errors:   string[];
}

export async function parseHistBoxScoreWithDeepSeek(pdfText: string, teamNameOverride?: string): Promise<HistParseResult> {
  ...
  const teamHint = teamNameOverride
    ...
    : `Extrai as estatísticas históricas do boxscore abaixo para a equipa CD Póvoa.`;
...
${OUTPUT_SCHEMA}`;
  ...
  const res = await fetch(DEEPSEEK_ENDPOINT, {
    ...
    body:    JSON.stringify({
      ...
      max_tokens: 8192,
      messages:   [{ role: "system", content: buildSystemPrompt(teamNameOverride) }, { role: "user", content: userContent }],
    }),
  });
  ...
  const tokLog = data.usage ? `${data.usage.prompt_tokens}p+${data.usage.completion_tokens}c` : "?";

  const players = ((raw.players ?? []) as Record<string, unknown>[]).map(normalizePlayer);

  const gameMeta: HistGameMeta = {
    team_name:     toStr(raw.team_name),
    opponent_name: toStr(raw.opponent_name),
    game_date:     parseDate(raw.game_date),
    home_score:    raw.home_score != null ? toInt(raw.home_score) : null,
    away_score:    raw.away_score != null ? toInt(raw.away_score) : null,
  ...
  return {
    players,
    gameMeta,
    aiLog: `deepseek ${tokLog} finish=${choice.finish_reason}`,
    errors,
  };
}

export async function parseHistBoxScoreFromImage(
  imageBuffer: Buffer,
  mimeType: string,
  teamNameOverride?: string,
): Promise<HistParseResult> {
...
═══ GAME METADATA ═══════════════════════════════════════════════════════════════
Also extract: game_date (ISO YYYY-MM-DD), opponent_name, final scores, competition name.
...
${OUTPUT_SCHEMA}`;
  ...
  if (!content || content.type !== "text") throw new Error("Claude: resposta sem conteúdo");
  ...
  return {
    players,
    gameMeta,
    aiLog: `claude-haiku vision ${tokLog} stop=${response.stop_reason}`,
    errors,
  };
}
