...
function buildRooms(entries: Entry[]): Room[] {
  ...
  if (recent.length) {
    ...
    rooms.push({ label: `Sala ${Math.min(...ys)}–${Math.max(...ys)}`, items: recent.map(e => ({ ...e, pieceNo: no++ })) });
  }
  if (older.length) {
    const ys = older.map(e => e.yr);
    rooms.push({ label: `Sala ${Math.min(...ys)}–${Math.max(...ys)}`, items: older.map(e => ({ ...e, pieceNo: no++ })) });
  }
  return rooms;
}

/* ─── Trophy card ───────────────────────────────────────────────────────── */
function TrophyCard({ piece, isAdmin, onOpen, onDelete }: {
  piece: Piece; isAdmin: boolean; onOpen: () => void; onDelete: () => void;
}) {
  ...
  return (
    <div className="group relative flex flex-col items-center w-full">
      ...
      >
        <div className="w-4/5 h-16 -mb-6 rounded-full" style={{ background: "radial-gradient(ellipse 80px 60px at 50% 20%, rgba(201,162,75,0.30) 0%, rgba(201,162,75,0.04) 60%, transparent 85%)" }} />
        <div className="relative z-10 flex items-end justify-center h-32 transition-transform duration-300 group-hover:-translate-y-2" style={{ filter: `drop-shadow(0 8px 16px ${tier.solid}40)` }}>
...
function TrophyModal({ piece, onClose }: { piece: Piece; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.88)" }} onClick={onClose}>
      <div className="relative flex flex-col items-center w-full max-w-xs rounded-lg p-8 pb-7"
        style={{ background: "linear-gradient(to bottom, #181610, #0d0b08)", border: "1px solid rgba(201,162,75,0.28)", boxShadow: "0 0 80px rgba(201,162,75,0.12), 0 32px 64px rgba(0,0,0,0.6)" }}
        onClick={e => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-3 right-3 p-1.5 rounded-full hover:bg-white/10 transition-colors" style={{ color: "#6f6a5f" }} aria-label="Fechar">
          <X className="h-4 w-4" />
        </button>
        <div className="absolute top-0 left-0 right-0 rounded-t-lg pointer-events-none" style={{ height: 100, background: `radial-gradient(ellipse 160px 90px at 50% 0%, ${tier.solid}50, transparent 70%)` }} />
        <div className="relative z-10 mt-6" style={{ filter: `drop-shadow(0 12px 24px ${tier.solid}50)` }}>
...
function AddModal({ onClose, onSaved }: { onClose: () => void; onSaved: (e: Entry) => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const res  = await fetch("/api/palmares", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ epoca: epoca.trim(), competicao, colocacao }) });
  ...
  const iS: React.CSSProperties = { background: "#0d0b08", border: "1px solid rgba(201,162,75,0.28)", borderRadius: 4, color: "#f5f1e8", padding: "8px 12px", fontSize: 14, width: "100%", outline: "none" };
  const lS: React.CSSProperties = { color: "#6f6a5f", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", display: "block", marginBottom: 6 };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.88)" }} onClick={onClose}>
      <form onSubmit={submit} className="relative w-full max-w-sm rounded-lg p-8"
        style={{ background: "linear-gradient(to bottom, #181610, #0d0b08)", border: "1px solid rgba(201,162,75,0.28)", boxShadow: "0 0 60px rgba(201,162,75,0.10), 0 24px 48px rgba(0,0,0,0.6)" }}
        onClick={e => e.stopPropagation()}>
        ...
        <p className="palmares-serif" style={{ color: "#c9a24b", fontSize: 22, fontWeight: 600, marginBottom: 24 }}>Adicionar Troféu</p>
        <div style={{ marginBottom: 16 }}>
          <label style={lS}>Época</label>
          <input style={iS} placeholder="ex: 2023/24" value={epoca} onChange={e => setEpoca(e.target.value)} autoFocus />
        </div>
        <div style={{ marginBottom: 16 }}>
          <label style={lS}>Competição</label>
          <select style={iS} value={competicao} onChange={e => setCompeticao(e.target.value as "Liga"|"Taça")}>
            <option value="Liga">Liga</option>
            <option value="Taça">Taça</option>
          </select>
        </div>
        <div style={{ marginBottom: 24 }}>
          <label style={lS}>Colocação</label>
          <select style={iS} value={colocacao} onChange={e => setColocacao(Number(e.target.value) as 1|2|3)}>
        ...
        <button type="submit" disabled={saving} className="w-full py-2.5 rounded transition-all"
          style={{ background: "rgba(201,162,75,0.18)", border: "1px solid rgba(201,162,75,0.45)", color: "#c9a24b", fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 14, letterSpacing: "0.18em", textTransform: "uppercase", cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>
...
function ConfirmDelete({ piece, onClose, onDeleted }: { piece: Piece; onClose: () => void; onDeleted: (id: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.88)" }} onClick={onClose}>
      <div className="relative w-full max-w-sm rounded-lg p-8 text-center"
        style={{ background: "linear-gradient(to bottom, #181610, #0d0b08)", border: "1px solid rgba(239,68,68,0.28)", boxShadow: "0 0 60px rgba(239,68,68,0.08), 0 24px 48px rgba(0,0,0,0.6)" }}
        onClick={e => e.stopPropagation()}>
        <button type="button" onClick={onClose} className="absolute top-3 right-3 p-1.5 rounded-full hover:bg-white/10 transition-colors" style={{ color: "#6f6a5f" }}><X className="h-4 w-4" /></button>
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={doDelete} disabled={deleting} className="flex-1 py-2 rounded"
            style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.4)", color: "#f87171", fontSize: 13, cursor: deleting ? "not-allowed" : "pointer", opacity: deleting ? 0.6 : 1 }}>
    ...
    <>
      ...
      <div className="palmares-root -mx-4 md:-mx-6 -mt-6 px-4 md:px-6 pb-20 pt-10 min-h-screen">
        ...
        <div className="text-center mb-10">
          ...
          <p className="palmares-serif" style={{ color: "#c9a24b", fontSize: 11, letterSpacing: "0.35em", textTransform: "uppercase", marginBottom: 6 }}>
            CD Póvoa Masters · Liga Amadora INATEL Porto
          </p>
          <h1 className="palmares-serif" style={{ color: "#f5f1e8", fontSize: "clamp(2.6rem, 7vw, 4rem)", fontWeight: 600, lineHeight: 1 }}>Palmarés</h1>
          <p className="palmares-serif" style={{ color: "#a8a296", fontSize: 18, marginTop: 6, fontStyle: "italic" }}>Sala de troféus — uma história de conquistas</p>
        ...
        {isAdmin && (
          <div className="flex justify-center mb-8">
            <button type="button" onClick={() => setShowAdd(true)} className="flex items-center gap-2 px-5 py-2 rounded transition-all hover:bg-white/8"
              style={{ border: "1px solid rgba(201,162,75,0.38)", color: "#c9a24b", fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 13, letterSpacing: "0.2em", textTransform: "uppercase" }}>
        ...
        {/* Loading */}
        {loading && <p className="palmares-serif text-center py-24" style={{ color: "#6f6a5f", fontSize: 20 }}>A carregar…</p>}
        ...
        {!loading && !!entries.length && (
          <>
            ...
            <div className="grid gap-5 max-w-3xl mx-auto" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 220px), 1fr))" }}>
              {current.items.map(piece => (
                <TrophyCard key={piece.id} piece={piece} isAdmin={isAdmin} onOpen={() => setSelected(piece)} onDelete={() => setToDelete(piece)} />
              ))}
            </div>
          </>
        )}
      ...
      {toDelete && <ConfirmDelete piece={toDelete} onClose={() => setToDelete(null)} onDeleted={handleDeleted} />}
    </>
  );
}
