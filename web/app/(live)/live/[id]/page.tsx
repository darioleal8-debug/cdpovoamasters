...
export default function SpectatorPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    const [sessRes, statsRes, playsRes] = await Promise.all([
      ...
      supabase.from("player_game_stats").select("*").eq("game_session_id", params.id),
    ...
    const rawStats = (statsRes.data ?? []) as (PlayerGameStatsWithUser & { player_id: string })[];
    ...
    setStats(rawStats.map((s) => ({ ...s, user: { id: s.player_id, name: nameById.get(s.player_id) ?? "?" } })) as PlayerRow[]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-cdpovoa-primary p-4 space-y-4">
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-cdpovoa-primary flex items-center justify-center text-white">
  ...
  return (
    <div
      ...
      style={{ minHeight: "100dvh", background: "var(--cdpovoa-primary, #0A1220)" }}
    >
      {/* ── Scoreboard ─────────────────────────────────── */}
      <div className="sticky top-0 z-10 border-b border-white/10 px-4 py-4"
        style={{ background: "var(--cdpovoa-primary, #0A1220)" }}>
