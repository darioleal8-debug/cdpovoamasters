...
export async function POST(req: NextRequest) {
  ...
  const { amount, description, category, entry_date, season_id, notes, player_name } = body as Record<string, unknown>;
  ...
  if (!entry_date)  return NextResponse.json({ error: "Data é obrigatória" },       { status: 400 });
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      player_name: player_name ? String(player_name) : null,
    })
    .select()
    .single();
